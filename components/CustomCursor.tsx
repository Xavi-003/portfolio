...
const CustomCursor: React.FC = () => {
  ...
  const springConfig1 = { damping: 30, stiffness: 400, mass: 0.1 };
  const springConfig2 = { damping: 28, stiffness: 300, mass: 0.2 };
  const springConfig3 = { damping: 25, stiffness: 200, mass: 0.3 };
  const springConfig4 = { damping: 22, stiffness: 150, mass: 0.4 };
  ...
  useEffect(() => {
    ...
    // Throttle mouseover to reduce state updates
    let lastMouseOverTime = 0;
    const mouseOver = (e: MouseEvent) => {
      const now = Date.now();
      if (now - lastMouseOverTime < 100) return; // Throttle to 100ms
